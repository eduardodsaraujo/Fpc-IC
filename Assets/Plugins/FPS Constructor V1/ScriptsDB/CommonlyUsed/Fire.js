...
var gameManagerJs : GameObject;
var traceEvery : int = 1;
var aO : AudioSource[];
var audioFire : boolean;

function Start () {
	effectsManager = GameObject.FindWithTag("Manager").GetComponent(EffectsManager);
	gameManagerJs = GameObject.FindWithTag("Scene");
	aO = gameManagerJs.GetComponent(GameManagerJs).aO;
}
...
//		aO = audioObj.AddComponent(AudioSource);
//		aO.clip = audio.clip;
//		aO.volume = audio.volume;
//		aO.pitch = audio.pitch;

	 for (var ao: AudioSource in aO) {
    // Do whatever...
	    if (!ao.isPlaying) {
			ao.Play();
			ao.loop = false;
